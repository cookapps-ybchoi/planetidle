...
public class ExcelImporter : AssetPostprocessor
{
	...
	static object CellToFieldObject(ICell cell, FieldInfo fieldInfo, bool isFormulaEvalute = false)
	{
		...
		if (fieldInfo.FieldType.IsArray)
		{
			...
			// 숫자 타입 배열 처리
			if (type == CellType.Numeric && elementType == typeof(float))
			{
				var array = Array.CreateInstance(elementType, 1);
				array.SetValue(Convert.ToSingle(cell.NumericCellValue), 0);
				return array;
			}
			else if (type == CellType.Numeric && elementType == typeof(int))
			{
				var array = Array.CreateInstance(elementType, 1);
				array.SetValue(Convert.ToInt32(cell.NumericCellValue), 0);
				return array;
			}
			else if (type == CellType.Numeric && elementType == typeof(double))
			{
				var array = Array.CreateInstance(elementType, 1);
				array.SetValue(cell.NumericCellValue, 0);
				return array;
			}
			// 문자열 배열 처리
			else if (type == CellType.String)
			{
				var stringValues = cell.StringCellValue.Split(',');
				var array = Array.CreateInstance(elementType, stringValues.Length);

				for (int i = 0; i < stringValues.Length; i++)
				{
					var trimmedValue = stringValues[i].Trim();
					if (string.IsNullOrEmpty(trimmedValue)) continue;

					if (elementType.IsEnum)
					{
						array.SetValue(Enum.Parse(elementType, trimmedValue), i);
					}
					else if (elementType == typeof(float))
					{
						array.SetValue(float.Parse(trimmedValue), i);
					}
					else if (elementType == typeof(int))
					{
						array.SetValue(int.Parse(trimmedValue), i);
					}
					else if (elementType == typeof(double))
					{
						array.SetValue(double.Parse(trimmedValue), i);
					}
					else
					{
						array.SetValue(Convert.ChangeType(trimmedValue, elementType), i);
					}
				}
				return array;
			}
	...
	static object CreateEntityFromRow(IRow row, List<string> columnNames, Type entityType, string sheetName)
	{
		...
		for (int i = 0; i < columnNames.Count; i++)
		{
			...
			// 배열 타입이고 셀이 비어있는 경우 빈 배열 생성
			if (cell == null && entityField.FieldType.IsArray)
			{
				var elementType = entityField.FieldType.GetElementType();
				var emptyArray = Array.CreateInstance(elementType, 0);
				entityField.SetValue(entity, emptyArray);
				continue;
			}
			...
			try
			{
				...
				// 배열 타입이고 문자열이 비어있는 경우 빈 배열 생성
				if (entityField.FieldType.IsArray && cell.CellType == CellType.String && string.IsNullOrEmpty(cell.StringCellValue))
				{
					var elementType = entityField.FieldType.GetElementType();
					var emptyArray = Array.CreateInstance(elementType, 0);
					entityField.SetValue(entity, emptyArray);
			...
			{
				throw new Exception(string.Format("엑셀 데이터 변환 오류: {0} 시트의 {1}행, {2}열 ({3} 필드) - {4}", 
					sheetName, row.RowNum + 1, cell.ColumnIndex + 1, columnNames[i], ex.Message));
			}
		}
		return entity;
	}
